...
#include <linux/mutex.h>     /* K6: k2u transaction serialization */
#include <linux/spinlock.h>  /* K6: slot/flag consistency vs the reply path */
...
/* K6 (2026-06 audit): the K2U request/response slot used to be a bare global —
 * concurrent senders (netlink handler, pcm_open's volume query, pcm_close's
 * rate-applied notify, kcontrol .put — all process context) overwrote each
 * other's slot and stole each other's wakeups, and a LATE reply (after a
 * sender's timeout) scribbled into the dead sender's stack rx_msg and then
 * satisfied the NEXT waiter with the wrong payload. Two locks close it:
 *  - k2u_transaction_mutex serializes whole transactions (arm -> send -> wait
 *    -> disarm), so exactly one request is ever in flight;
 *  - k2u_slot_lock makes slot+flag updates atomic against the receiver (which
 *    runs in the daemon's sendmsg context): a reply finding the slot DISARMED
 *    (sender gone: timeout/interrupt/send-failure) is dropped, not written
 *    through a stale pointer, and cannot wake a future waiter. */
...
int send_msg_to_user_land(struct MT_ALSA_msg* tx_msg, struct MT_ALSA_msg* rx_msg)
{
    ...
    /* K6: one transaction at a time; arm the slot under the slot lock, disarm
